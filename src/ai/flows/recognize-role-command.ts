...
const RecognizeRoleCommandOutputSchema = z.object({
    role: z.enum(['field_engineer', 'manager', 'user', 'unknown']).describe("The role identified from the voice command. It can be 'field_engineer', 'manager', 'user', or 'unknown'.")
});
...
const prompt = ai.definePrompt({
    ...
    prompt: `You are a voice command parser for a login system. Your task is to identify the user's desired role from their speech. The only valid roles are "field_engineer", "manager", and "user".
    ...
    - If the user mentions "user" or "standard user", the role is "user".
